...
| `A3.2` | AI-Shipping-Labs/website | Extension models for the remaining site-only user fields | A3.1 | no | review | https://github.com/AI-Shipping-Labs/website/issues/1692 — PASSED an independent re-gate after a first FAIL. Head a4015523 on c05a1c7e; scoped Django 15180 with 0 failures, test-core 3824 with 0, migration safety 0 of 49, and full Playwright 2604 passed 0 failed at load 8 to 9, the first trustworthy browser run on this branch. The tester verified the fix was complete as well as right: all 12 strategy models enumerated both ways, and _repoint_with_unique_keys has no third branch, so only kept-row strategies can strand contents. UNMERGED and waiting on one thing only: A3.1's human acceptance criterion |
