...
| `A3.2` | AI-Shipping-Labs/website | Extension models for the remaining site-only user fields | A3.1 | no | in-progress | https://github.com/AI-Shipping-Labs/website/issues/1692 — A3.2a and A3.2b rebased onto main as `a3.2-extension-models-rebased` (137708fc), A3.2c as `a3.2c-credentials-rebased` (a3dc2b4f), kept separate. Migrations renumbered to accounts 0032 and 0033 around the ones #1732 landed; the triggers/0003 ordering edge was proven load-bearing by deleting it and watching a fresh migrate die with LookupError. Zero new failures against a same-commit baseline on both. Still owes the P14 rehearsal and A3.1's two human criteria |
...
| `C7.12` | community-base | Conversion scripts and the unified format release | C7.9c, C7.10, C7.11 | no | in-progress | branch `c7.12-conversion-scripts`. D34, D38 and D39 applied to FORMAT.md and the registry first: the format contradicted all three and the scripts could not be written against it. Both converters exist, are idempotent and refuse rather than guess; run read-only against all sixteen real repositories, results in docs/plan/evidence/conversion-runs-2026-09-18.md. Six of the eight course repositories pass check_content with zero errors and zero warnings; the other two refuse on content the conversion will not author. The release tag is the owner's |
...
| `C7.18` | community-base | Make the code match decisions D38 and D39 | C7.9b, C7.10 | no | in-progress | branch `c7.12-conversion-scripts`, first commit. Taken inside C7.12 because the converters could not be written against a format the registry contradicted. `archive` is a mapping with a defaulting `notice_path`, read from the file rather than the defaulted values; the repository-file destination resolves to the ContentSource's GitHub repository at the synced commit, with no new setting. Course reference resolution can now be switched on: all eight real course repositories resolve their `code/`, `.py`, `.ipynb` and sibling-cohort links |
