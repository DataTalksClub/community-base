...
| `A2.1` | AI-Shipping-Labs/website | Adopt the Studio shell | C2.4 | no | blocked | https://github.com/AI-Shipping-Labs/website/issues/1615 — substantial progress on branch `a2.1-studio-shell`: the studio_filters library collision resolved across 106 templates, and decorators, impersonation and global search deleted in favour of the package. C7.13 and C7.14 are verified working (353 mounted routes, 353 claimed, 0 errors; collapse correct at 51 destinations). The shell deletion is blocked on C7.15, five further gaps found by rendering real Studio pages through the package shell, the hard one being an unconditional messages region a site cannot replace. The donor dashboard is a separate PM call |
...
| `C7.15` | community-base | Studio shell: messages, banner, focus ring and per-destination test hooks | C7.14 | no | in-progress |  |
