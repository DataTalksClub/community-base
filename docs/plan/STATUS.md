...
| `D1.2ca` | DataTalksClub/website | Send the remaining datamailer mail through the package mail app and retire the datamailer client | D1.2b | no | blocked | https://github.com/DataTalksClub/website/issues/409 — website#372 branch issue-372 gates green; dev deploy waits on the same fully green main CI run as D0.1c (website#345: 2026-09-16 scheduled full regression red); the worker self-check grant is effective since 2026-09-13; aws-infra#56 (human sandbox-account IAM fix) blocks only future aws-infra applies, not this deploy ; 2026-09-17 diagnosis: quality and django are repaired and green locally on branch ci-repair-20260917, playwright is a separate programme of about 84 pre-existing failures that CI stopped observing (the last main run recorded green ran playwright in reused mode and executed no tests). See docs/plan/evidence/dtc-main-ci-diagnosis-2026-09-17.md |
...
| `A2.1` | AI-Shipping-Labs/website | Adopt the Studio shell | C2.4 | no | in-progress | https://github.com/AI-Shipping-Labs/website/issues/1615 — all package blockers cleared: C7.13, C7.14 and C7.15 are merged. Branch `a2.1-studio-shell` already carries the studio_filters rename across 106 templates and the decorator, impersonation and global-search moves. The shell deletion can now proceed; it needs a package release and a one-time remap of nine AISL nav test ids to studio-nav-<destination key>. The donor dashboard layout remains a separate PM call |
...
| `D7.1` | DataTalksClub/website | DTC wiki and docs onto the shared app | C7.2, C7.4 | no | in-progress | https://github.com/DataTalksClub/website/issues/406 — implemented on branch `d7.1-knowledge-base`, three commits. Route and sitemap contracts pass unchanged and the pinned inventories were not edited; full-suite failure-set diff against a same-commit baseline is one known order-dependent flake. Cannot merge until a package release containing C7.4 is tagged: against v0.4.7 the docs reader dies with TypeError upsert_page() got an unexpected keyword argument 'body_html' |
...
| `C7.11` | community-base | Coursework: homework manifests from cohort bindings | C7.10, C5.2h | no | in-progress |  |
