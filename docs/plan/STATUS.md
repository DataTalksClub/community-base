...
| `D3.1a` | DataTalksClub/website | Additive extension schema (courses.LearnerProfile + accounts_ext) | C5.2a | no | in-progress | https://github.com/DataTalksClub/website/issues/390 — built as branch `d3.1a-extension-schema` (444ebf55), 0 new failures against a same-commit baseline. Needs rebase onto main (245 commits ahead) and the P14 development-copy rehearsal |
| `D3.1b` | DataTalksClub/website | Switch course-platform readers to courses.LearnerProfile | D3.1a | no | in-progress | https://github.com/DataTalksClub/website/issues/391 — built as branch `d3.1b-course-readers` (5b1865bf), no migrations, 0 new failures. Regression test pins the rule via ast rather than the three filenames, which do not exist at the branch base |
| `D3.1c` | DataTalksClub/website | Switch identity-window readers to accounts_ext.IdentityState | D3.1b | no | in-progress | https://github.com/DataTalksClub/website/issues/392 — built as branch `d3.1c-identity-readers` (2d32c2c4), no migrations, 0 new failures |
| `D3.1d` | DataTalksClub/website | Remove the twelve moved fields from CustomUser (contract) | D3.1c | no | in-progress | https://github.com/DataTalksClub/website/issues/393 — built as branch `d3.1d-contract-fields` (20541c8b), the twelve fields and the constraint are gone, 0 new failures |
| `D3.1e` | DataTalksClub/website | Rename CustomUser to User (RenameModel, AUTH_USER_MODEL) | D3.1d | no | in-progress | https://github.com/DataTalksClub/website/issues/394 — built as branch `d3.1e-rename-user` (46946378), get_user_model() is accounts.User on accounts_user, 0 new failures. The in-place rewrite of accounts/0001_initial is required, not merely convenient: a RenameModel in a later migration fails because 0001 cannot render itself once AUTH_USER_MODEL resolves to a model it does not create |
