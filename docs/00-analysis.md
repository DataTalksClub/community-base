...
| Events | DB-owned: `Event`, `EventSeries`, `Host`, `EventRegistration`, `SeriesRegistration`, `SeriesOccurrenceOptOut`, `EventFeedback`, reminders, ICS, Zoom, banners, recap, recording | GitHub-owned `Event` with UUID plus numeric `public_id`; lifecycle; Q&A sessions; historical registration aggregates; accountless registration only in spec | none |
...
| Events with series, registration, reminders, ICS, Zoom | complete | GitHub read model, registration in spec | AISL | high; DTC keeps `public_id` as an extension |
