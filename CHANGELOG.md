...
- C7.15: the Studio shell gains the hooks an adopting site needs before it can delete its own shell. The messages region moves into a `studio_messages` block a site can override or empty, and the default region carries `data-testid="messages-region"`, a `data-message-tag` per message and a tint keyed on the message level, so success and error differ without an override; an unknown level keeps the neutral card look. `studio_banner` is a new empty block between `<main>` and the padded content column, where a full-bleed strip such as an environment-mismatch warning goes. Every sidebar link gains the package's own focus-visible ring and `data-testid="studio-nav-<destination key>"`, derived from the registry key so it survives a change of title, section, order or grouping. Styling stays inside the classes the shipped `studio.css` already carries, so no stylesheet rebuild and no site design tokens come with it. The quick-jump palette and the mobile sidebar scroll affordance are answered rather than built: they belong in the package, because the palette is the package's search endpoint under a second trigger and the package already claims Ctrl/Cmd-K, and they are scoped as `C7.16` (AI-Shipping-Labs/website#1615).
