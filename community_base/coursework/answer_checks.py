"""Answer correctness checks per question type and answer type."""

from community_base.coursework.answer_crypto import HomeworkAnswerCryptoError
from community_base.coursework.models import Answer, AnswerTypes, Question, QuestionTypes
...
def safe_split_to_int(value: str, delimiter: str = ","):
    return [int(item) for item in safe_split(value, delimiter)]
...
def is_any_free_form_answer_correct(user_answer: str, correct_answer: str) -> bool:
    return len(user_answer) > 0


def is_exact_string_answer_correct(user_answer: str, correct_answer: str) -> bool:
    return user_answer.lower() == correct_answer.lower()


def is_contains_string_answer_correct(user_answer: str, correct_answer: str) -> bool:
...
def is_answer_correct(question: Question, answer: Answer) -> bool:
    ...
    try:
        ...
        # A missing, malformed, or unavailable source answer must never partially
        # score a submission or expose the underlying answer/key failure.
        return False
