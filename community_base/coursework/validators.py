...
def validate_option_keys(option, idx):
    if "criteria" not in option:
        raise ValidationError(f"Option at index {idx} is missing required key 'criteria'.")
    if "score" not in option:
        raise ValidationError(f"Option at index {idx} is missing required key 'score'.")


def validate_option_types(option, idx):
    if not isinstance(option["criteria"], str):
        raise ValidationError(
            f"Option at index {idx}: 'criteria' must be a string,"
            f" not {type(option['criteria']).__name__}."
        )
    if not isinstance(option["score"], int):
        raise ValidationError(
            f"Option at index {idx}: 'score' must be an integer,"
            f" not {type(option['score']).__name__}."
        )


def validate_option_content(option, idx):
    if not option["criteria"].strip():
        raise ValidationError(f"Option at index {idx}: 'criteria' cannot be empty.")
